...
export async function POST(request: NextRequest) {
  ...
  if (!repository.charities.findById(charityId)) {
    return NextResponse.json({ error: "Select a valid charity." }, { status: 400 });
  }
  if (charityContributionPercent < 10 || charityContributionPercent > 100) {
    return NextResponse.json({ error: "Charity contribution must be between 10 and 100 percent." }, { status: 400 });
  }
